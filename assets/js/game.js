...
function startCredits(e){
  ...
  credit1.anchor.x = 0;
  credit1.anchor.y = 1;
  credit1.position.x = 125;
  credit1.position.y = 100;
  credits.addChild(credit1);
  credit1a = new PIXI.Text(" ielsen", {font: "25px Arial", fill:"yellow"});
  credit1a.anchor.x = 0;
  credit1a.anchor.y = 1;
  credit1a.position.x = 275;
  credit1a.position.y = 100;
  credits.addChild(credit1a);
  var tileM = new PIXI.extras.TilingSprite(PIXI.Texture.fromImage('./assets/img/tileM.png'), 100, 100);
  tileM.anchor.x = 1;
  tileM.anchor.y = 1;
  tileM.position.x = 400;
  tileM.position.y = 400;
  credits.addChild(tileM);
  var tileN = new PIXI.extras.TilingSprite(PIXI.Texture.fromImage('./assets/img/tileN.png'), 100, 100);
  tileN.anchor.x = 1;
  tileN.anchor.y = 1;
  tileN.position.x = 100;
  tileN.position.y = 400;
  credits.addChild(tileN);
  credit2 = new PIXI.Text("CS 413", {font: "25px Arial", fill:"yellow"});
  credit2.anchor.x = 0.5;
  credit2.anchor.y = 1;
  credit2.position.x = 200;
  credit2.position.y = 150;
  credits.addChild(credit2);
  credit3 = new PIXI.Text("Project 2 - Puzzles", {font: "25px Arial", fill:"yellow"});
  credit3.anchor.x = .5;
  credit3.anchor.y = 1;
  credit3.position.x = 200;
  credit3.position.y = 200;
  credits.addChild(credit3);
  credit4 = new PIXI.Text("15 Tile", {font: "25px Arial", fill:"yellow"});
  credit4.anchor.x = .5;
  credit4.anchor.y = 1;
  credit4.position.x = 200;
  credit4.position.y = 250;
  credits.addChild(credit4);
  credit5 = new PIXI.Text("Back to Menu", {font: "25px Arial", fill:"Blue"});
  credit5.anchor.x = .5;
  credit5.anchor.y = 1;
  credit5.position.x = 200;
  credit5.position.y = 350;
  credit5.interactive = true;
  credit5.on('mousedown', menuFromCredits);
  credits.addChild(credit5);
  createjs.Tween.get(tileM.position).to({x: 125, y: 100}, 1000);
  createjs.Tween.get(tileN.position).to({x: 275, y: 100}, 1000);
  title.removeChildren();
  stage.removeChild(title);
